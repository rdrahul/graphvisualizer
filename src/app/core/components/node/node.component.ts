import { Component, OnInit, Input, OnChanges, ChangeDetectionStrategy, ChangeDetectorRef, ViewChild, ElementRef , EventEmitter, Output } from '@angular/core';
...
export class NodeComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.messageService.messages$.subscribe( (message:Node) => {
  ...
  MouseUp(event : Event){
    ...
    try{
      ...
      this.dropped.emit( {previousNode : JSON.parse(data), newNode:this.node} );
    }catch(err){
      console.error(err);
    }
  ...
  CreateWall(event){
    ...
    if (this.messageService.GetMouseClicked() == true && !this.node.isEndNode && !this.node.isStartNode ) {
      this.node.isWall = !this.node.isWall;
    }
  }
