...
export default class DjikstraAlgrothim {
    ...
    start( nodes, startNode, endNode ){
        ...
        for (const node of nodes) { unvisitedNodes.push( ...node ); }
    ...
    getNeighbors( currentNode, nodes ) {
        ...
        if ( currentNode.row > 0 ) { neighbors.push( nodes[currentNode.row - 1 ][currentNode.col] ); }
        if ( currentNode.col > 0) { neighbors.push( nodes[currentNode.row][currentNode.col - 1 ]); }
        if ( currentNode.row < nodes.length - 1 ) { neighbors.push( nodes[currentNode.row + 1][currentNode.col]); }
        if ( currentNode.col < nodes[0].length - 1 ) { neighbors.push( nodes[currentNode.row][currentNode.col + 1] ); }
